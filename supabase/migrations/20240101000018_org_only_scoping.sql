...
-- 1. Nulls out existing instance_id values (backfill)
-- 2. Drops NOT NULL constraints
-- 3. Replaces (instance_id, *) unique indexes with (organization_id, *) ones
-- 4. Drops (instance_id, *) composite indexes
-- 5. Rewrites RLS insert policies that checked instance_belongs_to_org

-- ── 1. Backfill: clear instance_id on all CRM data ───────────────────────────
...
-- ── 2. Drop NOT NULL on instance_id ─────────────────────────────────────────
...
-- ── 4. Drop instance-composite performance indexes ────────────────────────────
